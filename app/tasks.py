...
def task_a1(data_root: str) -> str:
    ...
    """
    user_email = os.environ.get("USER_EMAIL") or "22f3002771@ds.study.iitm.ac.in"
...
def task_a8(data_root: str) -> str:
    """
    A8. Use Tesseract OCR to extract the credit card number from /data/credit_card.png.
    """
    ...
    try:
        ...
        credit_card = "".join(re.findall(r"\d+", extracted_text))[:16]
        
        with open(output_path, "w", encoding="utf-8") as f:
            f.write(credit_card)
        
        return "Credit card number extracted using Tesseract."
    except Exception as e:
        raise RuntimeError(f"Failed to extract credit card number using Tesseract: {e}")
...
def task_a9(data_root: str) -> str:
    ...
    openai_api_key = "eyJhbGciOiJIUzI1NiJ9.eyJlbWFpbCI6IjIyZjMwMDI3NzErMUBkcy5zdHVkeS5paXRtLmFjLmluIn0.KuNbI4G5EPZJjT3phMWmP5Snnc_jnzM2ydfU5tzmDAw"
    try:
        response = httpx.post(
